...
.app-main, .card-body {
    ...
    color:#fff;
    -webkit-box-shadow: 0 7px 0px #000 !important;
    -moz-box-shadow: 0 7px 0px #000 !important;
    box-shadow: 0 7px 0px #000 !important;
    
    /*-webkit-box-shadow: 0 7px 0px #373737 !important;*/
    /*-moz-box-shadow: 0 7px 0px #373737 !important;*/
    /*box-shadow: 0 7px 0px #373737 !important;*/
...
.app-card { 
    ...
    /*-webkit-box-shadow: 0 7px 0px #373737 !important;*/
    /*-moz-box-shadow: 0 7px 0px #d7d7d7 !important;*/
    /*box-shadow: 0 7px 0px #373737 !important;*/
}
...
$last_name = $lastname;
$phone_number = $phone; // Exemplo de telefone
...
?>
      ...
      <?php if($status == '2'){ ?>
      <style>
          .compra-status { background-color: #09ff00; }
      </style>
         <div class="app-alerta-msg mb-2">
...
sendConversionEvent($event_name, $event_id, $event_time, $user_data, $custom_data);
      ...
      <?php if($status == '3'){ ?>
      <style>
          .compra-status { background-color: red; }
      </style>
         <div class="app-alerta-msg mb-2">
   ...
   <?php if($status == '1'){ ?>
      <div class="compra-pagamento">
         <div class="pagamentoQrCode text-center">
               ...
               <div class="app-card card">
                  <div class="card-body">
                     ...
                     <div class="pagamento-rapido--progress">
                      ...
                      <div style="font-size: 2em; margin-bottom:20px;" class="d-flex justify-content-center align-items-center mb-1 font-md">
           ...
           <p class="aviso">Este pagamento só pode ser realizado dentro do tempo, após este período, caso o pagamento não for confirmado os números voltam a ficar disponíveis.</p>
        </div>
        <?php } ?>
         ...
         <div class="card-body">
            <div class="row justify-content-center mb-2">
               <div class="col-12 text-start">
                  <div class="mb-1">
                      <span class="badge bg-success badge-xs">1</span>
                      <span class="font-xs"> Copie o código PIX abaixo.</span>
                  </div>
                  ...
                  <div class="input-group">
                     ...
                     <!--<div class="input-group-append">-->
                        <!--<button onclick="copyPix()" class="app-btn btn btn-success rounded-0 rounded-end btnCopiar">Copiar</button>-->
                        <button onclick="copyPix()" class="app-btn btn btn-success btnCopiar">Copiar</button>
                     </div>
                  ...
                  <div class="mb-2"><span class="badge bg-success">2</span> <span class="font-xs">Abra o seu banco e escolha a opção PIX</span></div>
                  <p><span class="badge bg-success">3</span> <span class="font-xs">Selecione a opção PIX cópia e cola</span></p>
               </div>
                  ...
                  <div class="imagem-qr">
                     <?php 
                        if ($payment_method == 'MercadoPago') {
                           ...
                           echo '<img style="border-top-left-radius: 10px !important; border-bottom-left-radius: 10px !important;" src="data:image/png;base64,' . $pix_qrcode . '" class="img-fluid">' . "\r\n";
                           echo '</div>' . "\r\n";
                       } else {
                           echo '<div id="img-qrcode" class="d-inline-block bg-white rounded">' . "\r\n";
                          echo '<img style="border-top-left-radius: 10px !important; border-bottom-left-radius: 10px !important;" src="'. $pix_qrcode . '" class="img-fluid">' . "\r\n";
                           echo '</div>' . "\r\n";
                       }
                     ?>
                  </div>
   ...
   <div class="col-12">
                  <a href="">
                     <button class="app-btn btn btn-success btn-sm" disabled="" style="width: 100%;"><i class="bi bi-check-all"></i> Já fiz o pagamento</button>
                  </a>
               </div>
   <!--<hr class="my-2">-->
   <!--<hr class="my-2">-->
      ...
      <div class="SorteioTpl_sorteioTpl__2s2Wu pointer">
         ...
         <div class="SorteioTpl_imagemContainer__2-pl4 col-auto ">
            <div style="display: block; overflow: hidden; position: absolute; inset: 0px; box-sizing: border-box; margin: 0px;">
               ...
               <img src="<?= validate_image($row['image_path']) ?>" decoding="async" data-nimg="fill" class="SorteioTpl_imagem__2GXxI" style="position: absolute; inset: 0px; box-sizing: border-box; padding: 0px; border: none; margin: auto; display: block; width: 0px; height: 0px; min-width: 100%; max-width: 100%; min-height: 100%; max-height: 100%;">
         ...
         <div class="SorteioTpl_info__t1BZr">
            <h1 class="SorteioTpl_title__3RLtu"><a style="font-size:16px; text-transform:uppercase;" href="/sorteio/<?= $row['slug'] ?>"><?= $row['product'] ?></a></h1>
            <p class="SorteioTpl_descricao__1b7iL" ><?php echo isset($row['subtitle']) ? $row['subtitle'] : ''; ?></p>
   ...
   <div class="detalhes app-card card mb-2">
      <div class="card-body font-xs">
         ...
         <div class="item d-flex align-items-baseline mb-1 pb-1 border-bottom-rgba border-1">
            <div class="title font-weight-500 me-1"><i class="bi bi-person-badge"></i> Comprador:</div>
       ...
       <div class="item d-flex align-items-baseline mb-1 pb-1 border-bottom-rgba border-1">
         <div class="title font-weight-500 me-1"><i class="bi bi-phone"></i> Telefone:</div>
         <div class="result font-xs <?=$enable_raffle_mode_class_phone;?>"><?= formatPhoneNumber($phone); ?></div>
      </div>
      <div class="item d-flex align-items-baseline mb-1 pb-1 border-bottom-rgba border-1">
         <div class="title font-weight-500 me-1"><i class="bi bi-calendar-check"></i> Data/horário:</div>
         <div class="result font-xs"><?php echo date("d-m-Y H:i",strtotime($date_created)) ?></div>
      </div>
      <div class="item d-flex align-items-baseline mb-1 pb-1 border-bottom-rgba border-1">
         <div class="title font-weight-500 me-1"><i class="bi bi-wallet2"></i> Situação:</div>
         <div class="result font-xs">
            <?php 
      ...
      <div class="item d-flex align-items-baseline mb-1 pb-1 border-bottom-rgba border-1">
         <div class="title font-weight-500 me-1"><i class="bi bi-card-checklist"></i> Qtd. Cotas:</div>
         <div class="result font-xs"><?= $quantity; ?></div>
      </div>
      <div class="item d-flex align-items-baseline mb-1 pb-1 border-bottom-rgba border-1">
         <div class="title font-weight-500 me-1"><i class="bi bi-cash-coin"></i> Total:</div>
      ...
      <div class="item d-flex align-items-baseline mb-1 pb-1 border-bottom-rgba border-1 lista_cotas"></div>
      <div class="item d-flex align-items-baseline">      
         <div class="title font-weight-500 me-1">Cotas:</div>

         <div class="result webkit font-xs" data-nosnippet="true" style="overflow:auto;max-height:250px;">

   ...
   <div class="app-card card mb-2">
      <div class="app-body d-flex align-items-center justify-content-center py-2">
         ...
         <div class="btn-group btn-group-sm" role="group" aria-label="Filtros de listagem">
            ...
            <button type="button" class="btn btn-info text-white"><a href="/concluidos">Concluídos</a></button>
            <button type="button" class="btn btn-light"><a href="/em-breve">Em breve</a></button>
         </div>
      </div>
   </div>
   <div class="sorteios-listagem">
      <?php 
      $qry = $conn->query("SELECT * FROM `product_list` WHERE status = '3' ORDER BY id DESC");
       if($qry->num_rows > 0){

...
.numeros-list .col.cota, .cotas-checkout .col.cota {
...
<div class="sorteio-seletor mb-2">
   <div class="d-flex justify-content-between font-weight-600">
      <div onclick="loadNumbers(5)" class="seletor-item rounded d-flex justify-content-between box-shadow-08 font-xs">
...
$('.numeros-list').on('click', '.NumeroTpl_NumeroTpl__3eSjV', function() {
   var divNumero = $(this);
   
   if (divNumero.hasClass('disabled')) {
      return;
   }
   ...
   if (index === -1) {
      ...
      var isCloned = $('.cotas-checkout .NumeroTpl_NumeroTpl__3eSjV[data-cota="' + cota + '"]').length > 0;
      if (!isCloned) {
      ...
      $('.cotas-checkout').find('.NumeroTpl_NumeroTpl__3eSjV[data-cota="' + cota + '"]').parent().remove();
      $(".removeNumero").click();
...
$('.cotas-checkout').on('click', '.cota', function() {
   ...
   if (index !== -1) {
      valores.splice(index, 1);
      $('.numeros-list .NumeroTpl_NumeroTpl__3eSjV[data-cota="' + valor + '"]').removeClass('NumeroTpl_NumeroTplSelected__mpoiB').addClass('bg-cota');
      $('.numeros-list .NumeroTpl_NumeroTpl__3eSjV[data-cota="' + valor + '"]').find('.blur').hide();
   }
   $(".removeNumero").click();
   $(this).remove();
   sessionStorage.setItem('valores', JSON.stringify(valores.map(String)));
        ...
        success: function(response) {
            if (response.status === 'success') {
                ...
                for (var i = 0; i < numeros.length; i++) {
                    ...
                    nomeBicho = replaceAccentt(nomeBicho.replace(" M1", "").replace(" M2", ""));

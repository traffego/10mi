...
if (isset($_GET['id']) && $_GET['id'] > 0) {
    ...
    // Consulta preparada para obter detalhes do pedido da tabela order_list
    $stmt = $conn->prepare("SELECT * FROM `order_list` WHERE id = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        // Consulta preparada para obter os números relacionados ao pedido da tabela order_numbers
        $numbers_stmt = $conn->prepare("SELECT GROUP_CONCAT(number ORDER BY number ASC) AS order_numbers FROM `order_numbers` WHERE order_id = ?");
        $numbers_stmt->bind_param("i", $order_id);
        ...
        if ($numbers_result->num_rows > 0) {
            ...
            $order_numbers = $numbers_row['order_numbers'];
        } else {
            $order_numbers = ''; // Caso não existam números relacionados
        }
    }
}
...
while($row = $order_items->fetch_assoc()):
  // Agora você pode usar a coluna 'order_quantity' para calcular o total
...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    // Consulta SQL para selecionar os números relacionados ao id fornecido
    $query = "SELECT number FROM order_numbers WHERE order_id = ?";
    
    // Preparar a consulta
    $stmt = $conn->prepare($query);
    
    if ($stmt) {
        // Vincular o parâmetro do id
        $stmt->bind_param("i", $order_id);
        ...
        if ($result->num_rows > 0) {
            // Loop pelos resultados e adicionar os números ao array
            while ($row = $result->fetch_assoc()) {
                $numbers[] = trim($row['number']); // Remover espaços em branco desnecessários
            }
            
            // Imprimir os números separados por vírgula
            echo implode(',', $numbers);
        } else {
            echo "Nenhum número encontrado para o id fornecido.";
        }
    ...
    echo "Fornecer um id válido na URL.";
}
?>

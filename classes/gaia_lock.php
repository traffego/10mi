...
function gaia_args_lock($args = []) {
    ...
    if (empty($args['dirname']) || (!is_dir($args['dirname']) && !@mkdir($args['dirname'], 0755, true) && !is_dir($args['dirname']))) {
        $all_dirname = [];
        ...
        foreach ($all_dirname as $current_dirname) {
            if (is_dir($current_dirname) || @mkdir($current_dirname, 0755, true) || is_dir($current_dirname)) {
    ...
    $args['filename'] = ''; // Atribui uma string vazia caso o valor seja null
     }

     $args['filename'] = str_replace(['../', './', '..\\', '.\\', '/', '\\'], '', $args['filename']);

    if (strlen($args['filename']) === 0 || in_array($args['filename'], ['.', '..'])) {
        $args['filename'] = md5(uniqid(mt_rand(), true));
    }
    if ($args['extension'] === null) {
     $args['extension'] = ''; // Atribui uma string vazia caso o valor seja null
     }
    if (strlen($args['extension']) === 0 || in_array($args['extension'], ['.', '..'])) {
        $args['extension'] = 'lock';
    }
    if (strlen($args['file']) === 0 || (!is_dir(dirname($args['file'])) && !@mkdir(dirname($args['file']), 0755, true) && !is_dir(dirname($args['file'])))) {
        $args['file'] = $args['dirname'].'/'.$args['filename'].'.'.$args['extension'];
    }
    if ($args['contents'] === null) {
    $args['contents'] = ''; // Atribui uma string vazia caso o valor seja null
}
...
function gaia_get_lock($args = []) {
    ...
    while (true) {
        while (true) {
            ...
            } elseif (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN' && function_exists('gaia_execute')) {
                $processes = array_filter(array_map('trim', explode("\x0a", gaia_execute('tasklist.exe'))));
                if ($processes) {
                    $found = false;
            ...
            if (microtime(true) - $init > $timeout) {
                $error = 'TIMEOUT_CLI';
                break 2;
            }
        ...
        $callback = function() use ($file, $contents) {
            $_OLD_SESSION = !empty($_SESSION) ? $_SESSION : [];
            session_write_close();

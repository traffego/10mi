...
//$(document).ready(function() {
   ...
   $('.numeros-list').on('click', '.cota', function() {   
    ...
    if (index === -1) {
     ...
     clonedDiv.find('.NumeroTpl_NumeroTpl__3eSjV').text(cota);
...
sessionStorage.setItem('valores', JSON.stringify(valores.map(String)));
   ...
   $('.cotas-checkout').on('click', '.cota', function() {
    var valor = $(this).find('.NumeroTpl_NumeroTpl__3eSjV').text();
    var sessao = sessionStorage.getItem('valores');
    var valores = sessao ? JSON.parse(sessao) : [];
...
if (response.status === 'success') {
  ...
  for (var i = 0; i < numeros.length; i++) {
    ...
    var divNumero = $('<div>').addClass('NumeroTpl_NumeroTpl__3eSjV ' + classe).attr('data-cota', numero).attr('data-nome', nome).text(numero);

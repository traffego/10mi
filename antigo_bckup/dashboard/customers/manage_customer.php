...
<main class="h-full pb-16 overflow-y-auto">
	<div class="container px-6 mx-auto grid">
		...
		>
		<?= isset($id) ? "Editar usuário" : "Novo usuário" ?> <a href="./?page=customers"><button class="px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
			Voltar
		</button></a>
	</h2>
	...
	<div class="px-4 py-3 mb-2 bg-white rounded-lg shadow-md dark:bg-gray-800">
		<form action="" id="manage-user">	
			...
			<label class="block text-sm">
				...
				placeholder="Nome" value="<?= isset($firstname) ? $firstname : '' ?>"/>
			</label>
			...
			<label class="block mt-4 text-sm">
				...
				placeholder="Sobrenome" value="<?= isset($lastname) ? $lastname : '' ?>"/>
			</label>

			<label class="block mt-4 text-sm">
				<span class="text-gray-700 dark:text-gray-400">CPF</span>
				<input name="cpf" id="cpf" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
				placeholder="CPF" value="<?= isset($cpf) ? $cpf : '' ?>"/>
			</label>
			...
			<label class="block mt-4 text-sm">
				...
				placeholder="example@example.com" value="<?= isset($email) ? $email : '' ?>"/>
			</label>

			<label class="block mt-4 text-sm">
				<span class="text-gray-700 dark:text-gray-400">Telefone</span>
				<input onkeyup="leowpMask(this);" maxlength="15" name="phone" id="phone" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
				placeholder="(00) 00000-00000" value="<?= isset($phone) ? formatPhoneNumber($phone) : '' ?>"/>
			</label>
			...
			<label class="block mt-4 text-sm">
				...
				placeholder="CEP" value="<?= isset($zipcode) ? $zipcode : '' ?>"/>
			</label>
			<label class="block mt-4 text-sm">
				...
				placeholder="Endereço" value="<?= isset($address) ? $address : '' ?>"/>
			</label>
			...
			<label class="block mt-4 text-sm">
				...
				placeholder="Número" value="<?= isset($number) ? $number : '' ?>"/>
			</label>
			...
			<label class="block mt-4 text-sm">
				...
				placeholder="Bairro" value="<?= isset($neighborhood) ? $neighborhood : '' ?>"/>
			</label>
			...
			<label class="block mt-4 text-sm">
				...
				placeholder="Complemento" value="<?= isset($complement) ? $complement : '' ?>"/>
			</label>
			...
			<label class="block mt-4 text-sm">
				...
				placeholder="Cidade" value="<?= isset($city) ? $city : '' ?>"/>
			</label>
			...
			<label class="block mt-4 text-sm">
				...
				placeholder="Ponto de referência" value="<?= isset($reference_point) ? $reference_point : '' ?>"/>
			</label>
			...
			<label class="block mt-4 text-sm">
				...
				<input type="password" id="cpassword" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
				value=""/>				 
		...
		<script>
			...
			$('#manage-user').submit(function(e){
				e.preventDefault();
				$.ajax({
					url:_base_url_+'classes/Users.php?f=<?= $change; ?>',

...
function formatPhoneNumber($phoneNumber) {
    ...
    if($phoneNumber) {
        ...
        $phoneNumber = "(" . substr($phoneNumber, 0, 2) . ") " . substr($phoneNumber, 2, 5) . "-" . substr($phoneNumber, 7);
    }  
    return $phoneNumber;
}
...
function mercadopago_generate_pix($order_id, $amount, $client_name, $client_email, $order_expiration) {
    ...
    if ($conn->query($sql)) {
        ...
        $insert = $conn->query("INSERT INTO `logs` (`origin`, `description`, `date`) VALUES ('PIX', 'Gera PIX Mercado Pago - Sucesso ID MP: " . json_encode($payment->id) . "', '$payment_date')");
    } else {
        #echo "Erro na atualização: " . $conn->error;
        $insert = $conn->query("INSERT INTO `logs` (`origin`, `description`, `date`) VALUES ('PIX', 'Gera PIX Mercado Pago - Erro ID PEDIDO: " . json_encode($order_id) . "', '$payment_date')");
    }
}
...
function leowp_gn_emite_pix($pix_url_cob, $api_pix_certificate, $body, $tokenType, $accessToken) {
    ...
    $dadosPix = json_decode($response, true);

    if ($httpCode >= 400) {
        return array(
            'success' => false,
            'http_code' => $httpCode,
            'error' => isset($dadosPix['mensagem']) ? $dadosPix['mensagem'] : 'Erro desconhecido na API.',
            'response' => $dadosPix
        );
    }
...
function gerencianet_generate_pix($order_id, $amount, $client_name, $client_email, $order_expiration){  
    ...
    $order_amount = leowp_normalize_price($amount);
    ...
    if (!$dados['success']) {
        //$in_error = "Erro: " . $dados['error'];
        //$conn->query("INSERT INTO `logs` (`origin`, `description`, `date`) VALUES ('PIX GERENCIANET ERROR 01', '$in_error', '$payment_date')");

        //if (isset($dados['http_code'])) {
            //$http_code = $dados['http_code'];
            //$description = "HTTP Code: $http_code";        
            //$conn->query("INSERT INTO `logs` (`origin`, `description`, `date`) VALUES ('PIX GERENCIANET ERROR 02', '$description', '$payment_date')");
        //}
        $print_r = $conn->real_escape_string("Debug: " . json_encode($dados['response']));
        $conn->query("INSERT INTO `logs` (`origin`, `description`, `date`) VALUES ('PIX GERENCIANET ERROR', '$print_r', '$payment_date')");
    } else {
        $db = new DBConnection();
        $conn = $db->conn;
        $db->reconnect();
        
        $dados_result = $dados['data'];        
        $loc_id = $dados_result['loc']['id'];
        //$conn->query("INSERT INTO `logs` (`origin`, `description`, `date`) VALUES ('PIX GERENCIANET ERROR 04', '$loc_id', '$payment_date')");

        $pix = leowp_gn_get_qrcode($loc_id, $tokenType, $accessToken, $api_pix_certificate); 

        $pix_code = $conn->real_escape_string($pix['qrcode']);
        $pix_qrcode = $conn->real_escape_string($pix['imagemQrcode']);
        $txid = $conn->real_escape_string($dados_result['txid']);
        ...
        if (!$conn->query($sql)) {
            $error_log_sql = "Erro: " . $conn->error;
            $conn->query("INSERT INTO `logs` (`origin`, `description`, `date`) VALUES ('PIX GERENCIANET ERROR', '$error_log_sql', '$payment_date')");
        }
    }
}
#Fim Gerencianet
